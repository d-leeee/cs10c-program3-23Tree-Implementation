...
class Tree {
    ...
    public:
        ...
        // Mutator functions
        void insert(const string& value); // Insert a new value into the tree
        void remove(const string& value); // Remove a value from the tree

        // Accessor functions
        bool search(const string& value) const;     // Search for the existance of a value in the tree
    ...
    private:
        ...
        string reorderThreeKeys(Node* node, const string& value); // When a node is full and needs to insert, reorder the node and return the key to move up to parent (used by insert function)
        Node* prepareOperation(const string& value) const;        // Search for the node to access and mutate (used by search, insert, remove functions)
